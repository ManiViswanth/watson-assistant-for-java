...
public abstract class WatsonService {
    ...
    //TokenManagerは6.9.0からもってきて改変したもの
    ...
    // HttpUrl url = HttpUrl.parse(getEndPoint()).newBuilder().setPathSegment(0,
    // AUTHORIZATION).build();
    // Request request = RequestBuilder.get(url + PATH_AUTHORIZATION_V1_TOKEN)
    // .header(HttpHeaders.ACCEPT, HttpMediaType.TEXT_PLAIN).query(URL,
    // getEndPoint()).build();
    //
    ...
    public void setApiKey(String apiKey) {
        ...
        mTokenManager = new MisawaTokenManager(apiKey);
    }
     ...
     */
    protected void setAuthenticationOld(Builder builder) {
        if (getApiKey() == null) {
    ...
    protected void setAuthentication(final Builder builder) {
        ...
        if (mTokenManager != null) {
            String accessToken = mTokenManager.getToken();
    ...
    /**
     * Returns true iff the specified mimeType indicates a JSON-related content
     * type.
     * (e.g. application/json, application/json-patch+json,
     * application/merge-patch+json, etc.).
     * 
     * @param mimeType
     *            the mimetype to consider
    ...
    /**
     * Returns true iff the specified mimeType indicates a "Json Patch"-related
     * content type.
